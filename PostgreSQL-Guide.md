...
В таблице `documents` указано `vector(1024)` — это размерность модели **Multilingual-E5-large**, которая используется по умолчанию (см. `embedding-server/README.md`). Если ваша модель эмбеддинга возвращает векторы другой размерности — поменяй число в скобках.
