...
**ЕСЛИ В ОФИСЕ НЕТ ИНТЕРНЕТА — читай только этот раздел:** [Установка на ПК без интернета (оффлайн-бандл)](#установка-на-пк-без-интернета-оффлайн-бандл).

**Если интернет есть** — обычная пошаговая инструкция:
...
## Установка на ПК без интернета (оффлайн-бандл)

Это типовая ситуация: офисный ПК отрезан от интернета, обычный `pip install` там не работает. Решение — подготовить «бандл» дома (или на любой машине с интернетом), принести на флешке и установить оффлайн.

В репозитории для этого есть **два готовых скрипта**, ничего вспоминать не надо.

### Шаг 1. Дома (на ПК с интернетом)
...
# Собрать бандл (скачает все .whl в подпапку wheels, ~3 GB)
.\make-offline-bundle.ps1
```

Скрипт выведет «ГОТОВО» и количество скачанных пакетов. После этого:
- Скопируй **всю папку `embedding-server`** (с подпапкой `wheels` внутри) на флешку.
- Если на офисном ПК ещё нет модели **Multilingual-E5-large** — скопируй и её (~9 GB).

### Шаг 2. В офисе (на ПК без интернета)

1. Скопируй с флешки папку `embedding-server` куда удобно, например `C:\Users\<имя>\Desktop\embedding-server`.
2. Если ещё не сделано — положи модель в `C:\models\multilingual-e5-large`.
3. Открой PowerShell в папке `embedding-server`:

```powershell
cd C:\Users\<имя>\Desktop\embedding-server

# Один раз разрешить запуск скриптов
Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned

# Установить всё из локальных wheels (без интернета)
.\install-offline.ps1
```

Скрипт сам создаст venv, активирует его, поставит все пакеты из `./wheels`, выведет «УСТАНОВКА УСПЕШНА» и подскажет следующие команды.
...
Не закрывай окно — сервер работает, пока оно открыто.

### Что важно учесть

- **Версия Python дома и в офисе должна совпадать по major.minor.** Бандл, собранный на Python 3.12, ставится только на 3.12. На 3.11 не встанет (разные ABI у скомпилированных пакетов). У тебя в офисе стоит 3.12.10 — дома тоже бери 3.12.x.
...
- **Бандл одноразовый.** Если будешь обновлять `requirements.txt` — собирай заново.
- **HuggingFace-onlineverification в коде сервера отключена** — `server.py` уже выставляет `HF_HUB_OFFLINE=1` и `TRANSFORMERS_OFFLINE=1` при запуске. Модель грузится строго из локальной папки, без попыток ходить в интернет.
